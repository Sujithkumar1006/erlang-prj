...
rec_poly_eval(Coeffs, X) -> rec_poly_eval(Coeffs, X, 0, 1).
rec_poly_eval([], _, Acc,_) -> Acc;
rec_poly_eval([H | Coeffs], X, Acc, Power) ->
    rec_poly_eval(Coeffs, X, Acc + (H * Power), X * Power).
...
tuple_poly_eval(TupleCoeffs, X) ->
    non_rec_poly_eval(lists:map(fun({_,Last}) -> Last end, TupleCoeffs), X).
...
assoc_lookup(Key, Assoc, DefaultFn) ->
   Result = lists:keyfind(Key,1, Assoc),
   case Result of
        {_, Num} -> Num;
    _Other ->
        DefaultFn()
...
id_poly_eval(Assoc, IdCoeffs, X) ->
    ConvertCoeffs =
        fun
            ({num, Number}) -> Number;
            ({_, Atom}) -> assoc_lookup_0(Atom, Assoc)
        end,
    NumList = lists:map(ConvertCoeffs, IdCoeffs),
    non_rec_poly_eval(NumList, X).
